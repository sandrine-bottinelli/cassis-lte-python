...
Adapted from https://matplotlib.org/stable/gallery/units/basic_units.html
"""
...
from cassis_lte_python.utils.utils import velo2freq, freq2velo
...
mhz = BasicUnit('MHz')
mhz.add_conversion_factor(hertz, 1.e6)
hertz.add_conversion_factor(mhz, 1.e-6)

ghz = BasicUnit('GHz')
ghz.add_conversion_factor(hertz, 1.e9)
hertz.add_conversion_factor(ghz, 1.e-9)
...
def cos(x):
    if np.iterable(x):
        return [np.cos(val.convert_to(radians).get_value()) for val in x]
    else:
        return np.cos(x.convert_to(radians).get_value())

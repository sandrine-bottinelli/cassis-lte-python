...
class ModelConfiguration:
    ...
    def cpt_dict(self):
        return {cpt.name: cpt for cpt in self.cpt_list}

    def update_tag_list(self, new_tags, ref_cpt_list=None):
        ...
        for win in self.win_list:
            ...
            if tag in self.tag_list and win.plot_nb in self.win_nb_fit[tag] and len(win.x_fit) >= 3:
                ...
                win.in_fit = False
        self.win_list_fit = [win for win in self.win_list if win.in_fit]
    ...
    def check_tex_bounds(self):
        if self._tex.vary:
            ...
            if isinstance(self._tex.max, (float, int)) and self.tmax < self._tex.max:
                sp_tmax = [sp for sp in self.species_list if max(sp.pf[0]) == self.tmax]
                if len(sp_tmax) > 1:
